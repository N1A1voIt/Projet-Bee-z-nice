...
public double getExpensesByIdUser(int idUser){
   Query query = entityManager.createNativeQuery("SELECT COALESCE(SUM(dishePrice),0) as expenses FROM foodorder WHERE customerid = :idCustomer", Expenses.class);
